...
const messages = {
  en: {
    ...
    'settings.title': 'Settings', 'settings.close': 'Close settings', 'settings.closeHelp': 'Close help', 'settings.general': 'General', 'settings.ai': 'AI', 'settings.search': 'Search', 'settings.shortcuts': 'Shortcuts', 'settings.status': 'Status', 'settings.appearance': 'Appearance', 'settings.privacy': 'Privacy', 'settings.keyboard': 'Keyboard', 'settings.about': 'About',
    ...
    'settings.status.title': 'Server status', 'settings.status.desc': 'Link up to two servers and three endpoint checks per server. NOVA checks only the HTTP(S) addresses you approve.', 'settings.status.show': 'Show system pulse', 'settings.status.showDesc': 'Keep a compact health readout beneath your shortcuts.', 'settings.status.guideTitle': 'Connect an Alibaba Cloud price project', 'settings.status.guideDesc': 'No Alibaba Cloud account, AccessKey, SSH password, or panel login is needed here.', 'settings.status.step1Title': 'Open your price project in a normal browser tab.', 'settings.status.step1Body': 'Copy an address that loads without a login prompt. Use /health if your project has one; otherwise use its home page.', 'settings.status.step2Title': 'Add one server and choose Alibaba Cloud.', 'settings.status.step2Body': 'Give it a friendly name, for example “Alibaba Cloud · Pricing”.', 'settings.status.step3Title': 'Paste that address into Server health URL.', 'settings.status.step3Body': 'Click Connect & verify, then approve the browser request for that address.', 'settings.status.step4Title': 'Optional: add up to three named checks.', 'settings.status.step4Body': 'For example, add “Website · 443” with the same public health address.', 'settings.status.examples': 'Copy one of these patterns', 'settings.status.domainExample': 'Domain', 'settings.status.ipExample': 'Public IP', 'settings.status.guideNote': 'Only public HTTP/HTTPS health addresses are supported. NOVA cannot test SSH port 22, private-network hostnames, AccessKey, or passwords.', 'settings.status.add': 'Add server', 'settings.status.limit': 'Two servers keep the new tab calm.', 'settings.status.name': 'Server name', 'settings.status.healthUrl': 'Server health URL', 'settings.status.healthHint': 'A public HTTP(S) health endpoint, for example https://api.example.com/health', 'settings.status.provider': 'Provider preset', 'settings.status.ports': 'Port checks', 'settings.status.portName': 'Label', 'settings.status.portUrl': 'Health URL', 'settings.status.addPort': 'Add port check', 'settings.status.portLimit': 'Up to three checks per server.', 'settings.status.removeServer': 'Remove server', 'settings.status.removePort': 'Remove {name}', 'settings.status.connect': 'Connect & verify', 'settings.status.accessDenied': 'Browser access was not granted for this server.', 'settings.status.invalid': 'Add a valid HTTP(S) health URL first.',
    ...
    'status.kicker': 'System pulse', 'status.title': 'Operations', 'status.configure': 'Configure', 'status.unnamed': 'Unnamed server', 'status.healthy': 'Healthy', 'status.degraded': 'Degraded', 'status.offline': 'Offline', 'status.unknown': 'Not checked', 'status.checking': 'Checking', 'status.noLatency': '— ms', 'status.refreshServer': 'Refresh {name}', 'status.portsFor': 'Endpoint health for {name}', 'status.noPorts': 'No port checks linked', 'status.notLinked': 'Not linked', 'status.empty': 'Link a server health endpoint when a calm glance is more useful than another dashboard.',
  ...
  'zh-CN': {
    ...
    'settings.title': '设置', 'settings.close': '关闭设置', 'settings.closeHelp': '关闭说明', 'settings.general': '通用', 'settings.ai': 'AI', 'settings.search': '搜索', 'settings.shortcuts': '快捷方式', 'settings.status': '运行状态', 'settings.appearance': '外观', 'settings.privacy': '隐私', 'settings.keyboard': '键盘', 'settings.about': '关于',
    ...
    'settings.status.title': '服务器状态', 'settings.status.desc': '最多连接两台服务器，每台最多三项端口检测。NOVA 只检测你明确授权的 HTTP(S) 地址。', 'settings.status.show': '显示系统脉冲', 'settings.status.showDesc': '在快捷网站下方保留精简的健康状态。', 'settings.status.guideTitle': '把阿里云上的价格项目接进来', 'settings.status.guideDesc': '这里不需要阿里云账号、AccessKey、SSH 密码或宝塔登录信息。', 'settings.status.step1Title': '先在普通浏览器标签页打开价格项目。', 'settings.status.step1Body': '复制一个不需要登录就能打开的网址。有 /health 就用它；没有就先用项目首页。', 'settings.status.step2Title': '添加一台服务器，选择“阿里云”。', 'settings.status.step2Body': '给它起个好懂的名字，例如“阿里云 · 价格项目”。', 'settings.status.step3Title': '把刚才的网址粘进“服务器健康地址”。', 'settings.status.step3Body': '点击“连接并验证”，再在浏览器弹窗中允许访问这个地址。', 'settings.status.step4Title': '可选：再添加最多 3 项命名检测。', 'settings.status.step4Body': '例如新增“网站 · 443”，填入同一个公开健康地址即可。', 'settings.status.examples': '可以直接照着填', 'settings.status.domainExample': '有域名', 'settings.status.ipExample': '只有公网 IP', 'settings.status.guideNote': '这里只支持公开的 HTTP/HTTPS 健康网址，不能检测 SSH 22 端口、内网地址、AccessKey 或密码。', 'settings.status.add': '添加服务器', 'settings.status.limit': '最多两台，才能保持新标签页安静。', 'settings.status.name': '服务器名称', 'settings.status.healthUrl': '服务器健康地址', 'settings.status.healthHint': '公开 HTTP(S) 健康检查地址，例如 https://api.example.com/health', 'settings.status.provider': '厂商预设', 'settings.status.ports': '端口检测', 'settings.status.portName': '名称', 'settings.status.portUrl': '健康地址', 'settings.status.addPort': '添加端口检测', 'settings.status.portLimit': '每台服务器最多三项。', 'settings.status.removeServer': '移除服务器', 'settings.status.removePort': '移除 {name}', 'settings.status.connect': '连接并验证', 'settings.status.accessDenied': '浏览器未授予此服务器的访问权限。', 'settings.status.invalid': '请先填写有效的 HTTP(S) 健康检查地址。',
    ...
    'status.kicker': '系统脉冲', 'status.title': '运行状态', 'status.configure': '配置', 'status.unnamed': '未命名服务器', 'status.healthy': '健康', 'status.degraded': '降级', 'status.offline': '离线', 'status.unknown': '未检测', 'status.checking': '检测中', 'status.noLatency': '— ms', 'status.refreshServer': '刷新 {name}', 'status.portsFor': '{name} 的端口健康状态', 'status.noPorts': '尚未连接端口检测', 'status.notLinked': '尚未连接', 'status.empty': '当一眼确认运行状态比再开一个面板更有价值时，在这里连接健康检查地址。',
