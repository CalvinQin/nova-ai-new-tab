...
- The extension requests Chrome `storage` by default. It declares optional HTTP/HTTPS host access solely for Server Pulse, and asks for the exact configured endpoint origins only after you select **Connect & verify**.
- Server Pulse stores only its display configuration (server name, provider, and endpoint URLs) locally. Once you approve an endpoint, it sends direct `GET` requests from your visible new-tab page at the cadence you choose (30 seconds by default; 1 minute, 5 minutes, or off are available), records only the resulting health label, latency, and check time locally, and never collects credentials or sends this information to NOVA.
...
- 扩展默认只申请 Chrome 的 `storage` 权限。为服务器状态面板声明了可选 HTTP/HTTPS 主机权限，但只有你点击“连接并检测”后，才会针对已配置端点的精确来源请求访问。
- 服务器状态面板只在本机保存展示配置（服务器名称、厂商和端点网址）；端点授权后，只要新标签页可见，浏览器就会按你选择的频率直接发起 `GET` 请求（默认每 30 秒，也可改为每 1 分钟、5 分钟或关闭），并只在本机记录健康标签、延迟和检测时间，不收集凭证，也不会把这些信息发送给 NOVA。
